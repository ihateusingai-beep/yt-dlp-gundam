...
# Write the auto-generated version_info into build/ (gitignored) so we don't
# dirty the working tree on every CI build. The static `version_info.txt`
# checked into the repo root is preserved as the manual-pyinstaller fallback
# (see that file's header).
_version_info = PROJECT_ROOT / "build" / "version_info.txt"
...
# main.py — written under build/ so it doesn't dirty git. The static copy
# in the repo root is the manual-pyinstaller fallback (see its header).
VSVersionInfo(
  ffi=FixedFileInfo(
